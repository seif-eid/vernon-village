...
var Connection = /** @class */ (function () {
    ...
    Connection.prototype.lock = function (path, callback) {
        ...
        this.request({
            ...
            if (res.statusCode >= 400)
                return callback(new HTTPError(res));
            var xml = xml_js_builder_1.XML.parse(Buffer.isBuffer(body) ? Int8Array.from(body) : body);
            try {
                callback(null, {
    ...
    Connection.prototype.readdir = function (path, _options, _callback) {
        ...
        var _a = createPropFindBody(options), findProps = _a[0], extraProperties = _a[1];
        this.request({
            url: path,
            method: 'PROPFIND',
            headers: {
            ...
            try {
                ...
                var results = xml_js_builder_1.XML.parse(Buffer.isBuffer(body) ? Int8Array.from(body) : body)
                    ...
                    if (options.properties) {
                        var props_1 = el.find('DAV:propstat').find('DAV:prop');
                        var type = props_1.find('DAV:resourcetype').findIndex('DAV:collection') !== -1 ? 'directory' : 'file';
                        var result_1 = {
                            name: name,
                            creationDate: props_1.findIndex('DAV:creationdate') !== -1 ? new Date(props_1.find('DAV:creationdate').findText()) : undefined,
                            lastModified: new Date(props_1.find('DAV:getlastmodified').findText()),
                            type: type,
                            isFile: type === 'file',
                            isDirectory: type === 'directory',
                            size: props_1.findIndex('DAV:getcontentlength') !== -1 ? parseInt(props_1.find('DAV:getcontentlength').findText()) : 0,
                            href: hrefWithoutTrailingSlash,
                        };
                        if (extraProperties.length > 0) {
                            result_1.extraProperties = {};
                            extraProperties.forEach(function (extraProperty) {
                                if (props_1.findIndex("" + extraProperty.namespace + extraProperty.element) !== -1) {
                                    result_1.extraProperties[extraProperty.element] = (extraProperty.nativeType ?
                                        toNativeType(props_1.find("" + extraProperty.namespace + extraProperty.element).findText()) :
                                        props_1.find("" + extraProperty.namespace + extraProperty.element).findText());
                                }
                                else if (extraProperty.default) {
                                    result_1.extraProperties[extraProperty.element] = extraProperty.default;
                                }
                            });
                        }
                        return result_1;
    ...
    Connection.prototype.getProperties = function (path, _options, _callback) {
        var options = _callback ? _options : {};
        var callback = _callback ? _callback : _options;
        var _a = createPropFindBody(options), findProps = _a[0], extraProperties = _a[1];
        this.request({
            ...
            if (res.statusCode >= 400)
                return callback(new HTTPError(res));
            try {
                var properties = xml_js_builder_1.XML.parse(Buffer.isBuffer(body) ? Int8Array.from(body) : body)
                    .find('DAV:multistatus')
                    .find('DAV:response')
                    ...
                    return el.find('DAV:prop')
                        ...
                        .map(function (el) {
                        var name = el.name;
                        for (var _i = 0, extraProperties_1 = extraProperties; _i < extraProperties_1.length; _i++) {
                            var extraProperty = extraProperties_1[_i];
                ...
                for (var _i = 0, properties_1 = properties; _i < properties_1.length; _i++) {
                    var propArr = properties_1[_i];
                    for (var _a = 0, propArr_1 = propArr; _a < propArr_1.length; _a++) {
                        var prop = propArr_1[_a];
                        result[prop.name] = {
                            content: prop.value,
...
function createPropFindBody(options) {
    var findProps = new xml_js_builder_1.XMLElementBuilder('d:propfind', { 'xmlns:d': 'DAV:' });
    var xmlProp = findProps.ele('d:prop');
    ...
    if (options && options.extraProperties && options.extraProperties.length > 0) {
        extraProperties = options.extraProperties.filter(function (extraProperty) {
        ...
        if (extraProperties.length > 0) {
            uniqBy(options.extraProperties, function (extraProperty) { return extraProperty.namespaceShort; })
                .forEach(function (extraProperty) {
                if (extraProperty.namespaceShort !== 'd') {
                    findProps.attributes["xmlns:" + extraProperty.namespaceShort] = extraProperty.namespace;
                }
            });
            options.extraProperties.forEach(function (extraProperty) {
